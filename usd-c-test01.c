...
// Simple example program to take a usd layer, open it and print out all its
// contained prim paths and properties thereupon
int main(int argc, char** argv) {
    ...
    // Construct a PrimRange and iterators to perform iteration
    // These are reference counted objects, hence the RAII dance
    ...
    while (!done) {
        ...
        for (int i = 0; i < num_props; ++i) {
            ...
            // Decide whether the property is an Attribute or a Relationship
...
void print_attribute(usd_Attribute_t* attr, usd_TimeCode_t tc) {
    ...
    // can't handle this yet... (add more ifs)
    printf("\n");
    vt_Value_dtor(val);
}